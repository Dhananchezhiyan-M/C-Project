...
void print_student_count(int choice) {
    const char *filenames[] = {
        ...
        "Design Thinking_waiting_list.csv",
        "Machine learning_waiting_list.csv",
        "Data Science_waiting_list.csv",
        "Applied Mathematics_waiting_list.csv",
        "Robotics_waiting_list.csv",
        "Theoretical Physics_waiting_list.csv",
        "Data Architecture_waiting_list.csv",
        "Psychology_waiting_list.csv",
        "Film Appreciation_waiting_list.csv"
    };
                            ...
                            if (strcmp(wait_username, students[k].username) == 0) {
                                for (int l = 0; l < NO_COURSES; l++) {
                                    if (strcmp(students[k].courses[l].name, courses[i].sname) == 0) {
                                        if (courses[i].seats[c] > 0) {
                                            courses[i].seats[c]--;
                                            alloted_update(wait_username,c,courses[i].sname);
                                            reallocated = 1;
                                            break;
                                        }
                                    }
...
int allocate_courses(Student *students, int num_students, struct Core *courses, int subcount) {
    ...
    //The one where admin starts the first alloting
...
void sort_and_write_csv(const char *input_file, const char *output_file) {
    ...
    // Sort users by CGPA a normal bubble sort
            ...
            reallocate_waitlisted_students(students, num_students, courses, subcount);
             ...
             while(1) {
                 ...
                 if (choice>0 || choice<12) {
                     ...
                     printf("Wrong input..Enter a valid choice...");
                 }
             }
...
//for dropping and adding a new course for the students in the alloted csv
...
void update_waitlist_points(Student *students, char username[]) {
    ...
    if (!student_found) {
        printf(BOLD_RED "Student not found." RESET "\n");
        return;
    }
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (temp_waiting_file == NULL) {
            fclose(waiting_file);
            printf(BOLD_RED "Failed to create temporary file." RESET "\n");
            return;
        }
        ...
        if (remove(filename) != 0) {
            printf(BOLD_RED "Error deleting original file." RESET "\n");
            remove(temp_filename);
        } else if (rename(temp_filename, filename) != 0) {
            printf(BOLD_RED "Error renaming temporary file." RESET "\n");
        }
...
void modify_allotted_courses(User *user) {
    ...
    printf("Enter the number of the course you want to drop: ");
    scanf("%d", &drop_index);
    ...
    printf("Available core courses to add:\n");
    for (int i = 0; i < courseCount; i++) {
